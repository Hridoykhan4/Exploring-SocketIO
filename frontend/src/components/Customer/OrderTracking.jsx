        ...
        <div className="container mx-auto px-4 py-8">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
                    ...
                    {/* Current Status */}
                    <div className={`${getStatusColor(order.status)} text-white p-4 rounded-lg`}>
                        <p className="text-2xl font-bold">{getStatusMessage(order.status)}</p>
                        {order.estimatedTime && !['delivered', 'cancelled'].includes(order.status) && (
                            <p className="text-lg mt-2">Estimated time: {order.estimatedTime} minutes</p>
                        )}
                    </div>
                </div>
                            ...
                            <div className="relative flex justify-between">
                                {statusSteps.map((step, index) => (
                                    <div key={step.key} className="flex flex-col items-center">
                                        <div
                                            className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg transition-all ${index <= currentStepIndex
                ...
                <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
                    ...
                    <div className="space-y-3">
                        ...
                        <div>
                            ...
                            <p className="font-medium">{order.customerAddress || order.address}</p>
                        </div>
                        {(order.specialNotes || order.notes) && (
                            <div>
                                <p className="text-sm text-gray-600">Special Notes</p>
                                <p className="font-medium">{order.specialNotes || order.notes}</p>
                            </div>
                        )}
                    </div>
                </div>
