...
const AdminDashboard = ({ socket, onShowNotification, onLogout }) => {
    ...
    useEffect(() => {
        ...
        if (!adminPassword) {
            queueMicrotask(() => {
                setLoading(false);
                onShowNotification('Please login again', 'error');
                onLogout();
            });
            return;
        }
        ...
        socket.emit('adminLogin', { password: adminPassword }, (response) => {
            if (response.success) {
                ...
                setLoading(false);
                onShowNotification(response.message || 'Admin session expired', 'error');
                onLogout();
            }
        });
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-2 flex md:flex-col gap-1 min-w-max md:min-w-0">
                            {tabs.map(tab => (
                                <button
                                    ...
                                    className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all w-full text-left ${activeTab === tab.key
                                ...
                                >
                                    ...
                                    <span className={`px-2 py-0.5 rounded-md text-xs font-bold ${activeTab === tab.key ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-600'
                                        }`}>
